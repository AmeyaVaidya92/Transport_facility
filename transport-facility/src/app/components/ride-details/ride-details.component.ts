...
export class RideDetailsComponent {
  ...
  constructor(private auth: AuthService, private rides: RideService) {}

  canBook(): boolean {
    if (!this.ride) { return false; }
    const user = this.auth.getCurrentUser();
    if (!user) { return false; }
    if (this.ride.creatorId === user) { return false; }
    if (this.ride.bookings.includes(user)) { return false; }
    if (this.ride.vacantSeats <= 0) { return false; }
    return true;
  }
